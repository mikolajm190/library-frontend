...
export default function ReservationCreateForm({
  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-2 text-xs text-[color:var(--ink-muted)]">
          ...
          <input
            ...
            required
            disabled={isSubmitting}
            list="reservation-book-options"
            className="rounded-xl border border-black/10 bg-white px-3 py-2 text-sm text-[color:var(--ink)] outline-none transition focus:border-black/30 focus:ring-2 focus:ring-black/5"
          />
        </label>
      </div>
      <datalist id="reservation-user-options">
        {users.map((user) => (
          <option key={user.id} value={user.username} />
        ))}
      </datalist>
      <datalist id="reservation-book-options">
        {books.map((book) => (
      ...
      >
        {isSubmitting ? 'Creating...' : isLoading ? 'Loading...' : 'Create reservation'}
      </button>
    </form>
  )
}
